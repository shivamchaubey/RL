...
class DeepQ:
    ...
    def createModel(self):
        ...
        model = Sequential()
        model.add(Convolution2D(64, (3,3), strides=(2,2), input_shape=(img_channels,img_rows,img_cols)))
...
if __name__ == '__main__':
    ...
    rospy.init_node('vehicle_gym', anonymous=True)
    #REMEMBER!: turtlebot_cnn_setup.bash must be executed.
    env = gym.make('VehicleDQN-v0')
    ...
    plt.suptitle('Reward Plot')
    hist_reward = []            
    if not continue_execution:
        minibatch_size = 1
        learningRate = 1e-2#1e6
        discountFactor = 0.8
        network_outputs = 25
        memorySize = 100000
        learnStart = 10 # timesteps to observe before training
    ...
    for epoch in xrange(current_epoch+1, epochs+1, 1):
        ...
        for t in xrange(steps):
            ...
            if done:
                ...
                h, m = divmod(m, 60)
                hist_reward.append(round(cumulated_reward, 2))
                plt.plot(hist_reward)
                # plt.show()
                if not last100Filled:
                    print ("\n>>>>>>>>>>>>","EP "+str(epoch)+" - {} steps".format(t+1)+" - CReward: "+str(round(cumulated_reward, 2))+"  Eps="+str(round(explorationRate, 2))+"  Time: %d:%02d:%02d" % (h, m, s))
                
                else :
                    print ("\n>>>>>>>>>>>>","EP "+str(epoch)+" - {} steps".format(t+1)+" - last100 C_Rewards : "+str(int((sum(last100Rewards)/len(last100Rewards))))+" - CReward: "+str(round(cumulated_reward, 2))+"  Eps="+str(round(explorationRate, 2))+"  Time: %d:%02d:%02d" % (h, m, s))
