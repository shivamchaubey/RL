...
class vehicle_dqn(gym.Env):
    ...
    def __init__(self):
        ...
        ########################################################################
        max_acceleration=2.0 # max m/s2
        max_angle_steering=0.249 #Absolut value
        self.actions=[]
        for i in np.linspace(-0.5,max_acceleration,5): # 5 values of acceleration
            for j in np.linspace(-max_angle_steering,max_angle_steering,5): # / values of angle steering
